...
class GCode(object):
    ...
    def rewrite_layer(self, commands, layer_idx):
        ...
        self.line_idxs = self.line_idxs[:start_index] + array('I', list(range(len(commands)))) + self.line_idxs[end_index:]
    ...
    def _preprocess(self, lines = None, build_layers = False,
        ...
        if build_layers:
            ...
            # Finalize duration
            print(totalduration/60/60)
            totaltime = datetime.timedelta(seconds = int(totalduration))
            self.duration = totaltime
            self.duration_hours = totalduration/60/60
